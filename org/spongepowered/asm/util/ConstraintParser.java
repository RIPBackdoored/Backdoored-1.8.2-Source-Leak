...
public final class ConstraintParser
{
    ...
    public static Constraint parse(final String v-5) {
        if (v-5 == null || v-5.length() == 0) {
            return Constraint.NONE;
        }
        final String[] split = v-5.replaceAll("\\s", "").toUpperCase().split(";");
        Constraint constraint = null;
        for (final String v1 : split) {
            final Constraint a1 = new Constraint(v1);
            if (constraint == null) {
                constraint = a1;
            }
            else {
                constraint.append(a1);
            }
        }
    ...
    public static Constraint parse(final AnnotationNode a1) {
        final String v1 = Annotations.getValue(a1, "constraints", "");
        return parse(v1);
    }
